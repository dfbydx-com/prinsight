...
namespace Fe {
  ...
  void handleRegister(const Rest::Request&, Http::ResponseWriter response) {
    response.send(Http::Code::Ok, "pubkey");
  }
...
private:
  void setupRoutes() {
    ...
    Routes::Get(router, "/register", Routes::bind(&Fe::handleRegister));
